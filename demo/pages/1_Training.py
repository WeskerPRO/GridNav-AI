from matplotlib import pyplot as plt                   # loss / reward plots
import streamlit as st                                  # the web UI
import sys                                              # fix import path
import os                                               # path helpers
import io                                               # in-memory model download
import numpy as np                                      # (used indirectly by helpers)
import torch                                            # save model weights

sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))  # make 'core' importable
from core.grid_utils import render_grid_image, render_reward_curve, image_to_bytes  # viz helpers
from core.rl_model import train_rl_live                 # RL training generator
from core.supervised_model import train_supervised_live # supervised training generator

# =============================================================================
# PAGE CONFIG
# =============================================================================

st.set_page_config(                                     # browser tab + layout
...
""", unsafe_allow_html=True)                            # inject dark theme CSS

# =============================================================================
# SIDEBAR — training config
# =============================================================================

st.sidebar.markdown("## ⚡ TRAINING CONFIG")             # sidebar header
st.sidebar.markdown("---")

# ── algorithm selector ────────────────────────────────────────────────────────
train_mode = st.sidebar.radio(                          # choose RL or Supervised
    "Algorithm:",
...
if "RL" in train_mode:                                  # RL knobs
    density_min = st.sidebar.slider("Min obstacle density", 0.05, 0.30, 0.10, 0.05)  # easiest grids
    density_max = st.sidebar.slider("Max obstacle density", 0.15, 0.45, 0.35, 0.05)  # hardest grids
    episodes    = st.sidebar.slider("Episodes", 1000, 8000, 5000, 500)               # training length
    speed       = st.sidebar.slider("Update every N episodes", 5, 100, 20)           # UI refresh rate
    st.sidebar.markdown(f"""
    <p style="color:#533483; font-size:0.75rem;">
    DQN-LSTM agent:<br>
    • 7×7 vision window<br>
    ...
    """, unsafe_allow_html=True)                        # info blurb
else:                                                   # Supervised knobs
    density = st.sidebar.slider("Obstacle density", 0.10, 0.40, 0.20, 0.05)          # single density
    epochs = st.sidebar.slider("Epochs", 5, 50, 20)                                  # passes over data
    num_samples = st.sidebar.slider("Training samples", 1000, 10000, 3000, 500)      # BFS samples
    learning_rate = st.sidebar.select_slider(           # log-spaced LR picker
        "Learning rate",
        options=[1e-6, 1e-5, 5e-5, 1e-4, 5e-4, 1e-3, 5e-3, 1e-2, 5e-2],
        value=5e-4,                                  # good default for the ResNet
        format_func=lambda x: f"{x:.0e}",            # show as 5e-04 etc.
    )
    weight_decay = st.sidebar.select_slider(            # log-spaced L2 picker
        "Weight decay",
        options=[0.0, 1e-5, 1e-4, 5e-4, 1e-3, 5e-3, 1e-2],
        value=5e-3,                                  # matches path_finder.py default
        format_func=lambda x: "0 (off)" if x == 0 else f"{x:.0e}",
    )
...
st.markdown("# ⚡ LIVE TRAINING")                        # page title
if "RL" in train_mode:                                  # mode-specific intro
    st.markdown(
...
# ── layout ────────────────────────────────────────────────────────────────────
left_col, right_col = st.columns([1, 1])                # two side-by-side columns

with left_col:                                          # left: grid / loss
    if "RL" in train_mode:
        st.markdown("## 🗺️ CURRENT EPISODE GRID")
    else:
        st.markdown("## 📉 LOSS CURVE")
    grid_display  = st.empty()                          # placeholder for grid image
    st.markdown("## 📊 REWARD CURVE" if "RL" in train_mode else "")
    curve_display = st.empty()                          # placeholder for curve

with right_col:                                         # right: metrics + log
    st.markdown("## 📈 METRICS")
    m1, m2 = st.columns(2)                              # 2×2 metric grid
    m3, m4 = st.columns(2)
    metric_episode = m1.empty()                         # live metric slots
    metric_reward  = m2.empty()
    metric_epsilon = m3.empty()
    metric_loss    = m4.empty()
    st.markdown("---")
    st.markdown("## 📋 STATUS LOG")
    status_log = st.empty()                             # scrolling text log

progress_bar = st.progress(0)                           # 0..1 progress bar

# =============================================================================
# CONTROLS
# =============================================================================

ctrl1, ctrl2, _ = st.columns([1, 1, 3])                 # button row
start_btn = ctrl1.button("▶  START TRAINING")           # begin
stop_btn  = ctrl2.button("⏹  STOP")                      # request stop

if "training_active" not in st.session_state:           # init session flags
...
if start_btn:                                           # START pressed
    st.session_state.stop_requested  = False            # clear old stop
    st.session_state.training_active = True
    st.session_state.trained_model   = None

    log_lines = []                                      # status-log buffer
    ...
    if "RL" in train_mode:
        ...
        cell_sz = max(16, 400 // max(rows, cols))       # px per cell for rendering

        trainer = train_rl_live(                        # build the training generator
            rows            = rows,
            cols            = cols,
        ...
        for update in trainer:                          # consume yields
            if st.session_state.stop_requested:         # STOP mid-run?
                log_lines.append("⏹ Training stopped by user.")
                status_log.markdown("```\n" + "\n".join(log_lines[-8:]) + "\n```")
                break

            ep      = update["episode"]                 # unpack the progress dict
            ...
            cur_numeric = update["current_numeric"]     # current episode's grid
            cur_robot   = update["current_robot"]
            cur_target  = update["current_target"]

            # progress
            progress_bar.progress(ep / total)           # advance bar

            # metrics
            metric_episode.metric("Episode",   f"{ep} / {total}")     # update metrics
            ...
            if cur_numeric is not None:
                img = render_grid_image(                # draw grid + path
                    cur_numeric,
                    path[-1] if path else cur_robot,    # robot = end of path
                    cur_target,
                    path_taken = path,
                    cell_size  = cell_sz
                )
                grid_display.image(image_to_bytes(img), width='stretch')  # show it

            # reward curve
            fig = render_reward_curve(rewards, title=f"RL Reward — Episode {ep}")  # plot
            curve_display.pyplot(fig)
            plt.close(fig)                              # free figure memory

            # log
            if sr is not None:                          # eval happened this tick
                log_lines.append(
                    f"📊 eval ep {ep}: success={sr:.0f}% | "
                    f"early_stop {cs}/{pat}"
                )
            if ep % (speed * 5) == 0:                   # periodic stats line
                log_lines.append(
            ...
            if update["done_training"]:                 # generator finished
                if cs >= pat and sr is not None:        # stopped early?
                ...
                status_log.markdown("```\n" + "\n".join(log_lines[-10:]) + "\n```")
                st.session_state.trained_model = model  # keep the model
                st.success(msg)
                break
    ...
    # =========================================================================
        ...
        grid_display.info(                              # no live grid for supervised
        ...
        with st.spinner(                                # dataset gen can be slow
            f"Generating {num_samples} training samples from BFS... "
            "(this may take 30-60 seconds)"
        ):
            trainer = train_supervised_live(            # build the training generator
                rows,
                cols,
                obstacle_density = density,
                epochs = epochs,
                weight_decay = weight_decay,            # from the slider
                lr = learning_rate,                     # from the slider
                num_samples = num_samples,
                progress_every = 1
            )

        for update in trainer:                          # consume yields (per epoch)
            if st.session_state.stop_requested:         # STOP mid-run?
                log_lines.append("⏹ Training stopped by user.")
                status_log.markdown("```\n" + "\n".join(log_lines[-8:]) + "\n```")
                break

            ep       = update["epoch"]                  # unpack progress
            ...
            # metrics
            metric_episode.metric("Epoch",        f"{ep} / {total}")   # update metrics
            ...
            ax.plot(update["train_losses"], color='#E94560',
                    linewidth=2, label='Train Loss')    # train curve
            ax.plot(update["val_losses"],   color='#00B4D8',
                    linewidth=2, label='Val Loss')      # val curve
            ...
            plt.tight_layout()
            curve_display.pyplot(fig)                   # show curve
            plt.close(fig)                              # free figure memory

            # log
            log_lines.append(                           # append epoch stats
                f"epoch {ep:>3} | train={t_loss:.4f} | "
                f"val={v_loss:.4f} | acc={accuracy:.1f}%"
            )
            status_log.markdown("```\n" + "\n".join(log_lines[-10:]) + "\n```")

            if update["done_training"]:                 # finished
                msg = (
    ...
    st.session_state.training_active = False            # training done

# =============================================================================
# SAVE MODEL
# =============================================================================

if st.session_state.trained_model is not None:          # something to download?
    st.markdown("---")
    st.markdown("### 💾 SAVE MODEL")

    is_rl = isinstance(                                 # RL or supervised model?
        st.session_state.trained_model,
        __import__('core.rl_model', fromlist=['DQN_LSTM']).DQN_LSTM
    )
    filename = "gridnav_rl_trained.pth" if is_rl else "gridnav_supervised_trained.pth"  # name it

    buf = io.BytesIO()                                  # in-memory file
    torch.save(st.session_state.trained_model.state_dict(), buf)  # serialise weights
    buf.seek(0)
    st.download_button(                                 # download widget
        label     = f"⬇ Download trained model ({filename})",
        data      = buf,
        file_name = filename,
        mime      = "application/octet-stream"
    )
    if is_rl:                                           # mode-specific note
        st.info(
